...
- **Module Organization**: All plugins are organized under the `PRO_Extensions` namespace module
- **Resource Assets**: Each plugin includes its own icons and resources in subdirectories

### Key Plugins

1. **PRO_Anotacoes** - Annotation tools for room names and section cuts
2. **PRO_Iluminacao** - Dynamic lighting fixtures and points
3. **PRO_Interiores** - Interior design tools with scene management and layer organization
4. **PRO_Mobiliario** - Furniture dimensioning and quantification tools
5. **PRO_PontosTecnicos** - Technical points management
6. **PRO_RevestimentosRodapes** - Wall coverings and baseboards
...
The PRO_Interiores plugin provides automated scene creation and configuration with predefined views:
...
```ruby
# Main plugin file structure (e.g., PRO_PluginName.rb)
require 'sketchup.rb'
require 'extensions.rb'

module PRO_Extensions
  module PluginName
...
├── PRO_PluginName.rb             # Main plugin registration files
├── PRO_PluginName/               # Plugin implementation directories
